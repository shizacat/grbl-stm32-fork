...
#endif
{
  ...
  #endif
  #ifdef STM32F103C8
    HAL_GPIO_Write(DIRECTION_PORT, (HAL_GPIO_Read(DIRECTION_PORT) & ~DIRECTION_MASK) | (st.dir_outbits & DIRECTION_MASK));
    #ifdef ENABLE_DUAL_AXIS
  ...
  // Then pulse the stepping pins
    ...
    st.step_bits = (STEP_PORT & ~STEP_MASK) | st.step_outbits; // Store out_bits to prevent overwriting.
    ...
    #endif
    #ifdef STM32F103C8
      HAL_GPIO_Write(STEP_PORT, (HAL_GPIO_Read(STEP_PORT) & ~STEP_MASK) | st.step_outbits);
    #endif
      ...
      #endif
      #ifdef STM32F103C8
        HAL_GPIO_Write(STEP_PORT_DUAL, (HAL_GPIO_Read(STEP_PORT_DUAL) & ~STEP_MASK_DUAL) | st.step_outbits_dual);
      #endif
    #endif
  #endif
...
void TIM3_IRQHandler(void) {
  ...
  if(__HAL_TIM_GET_FLAG(&htim3, TIM_FLAG_UPDATE) != RESET)
    ...
    HAL_NVIC_DisableIRQ(TIM3_IRQn);
		HAL_GPIO_Write(STEP_PORT, (HAL_GPIO_Read(STEP_PORT) & ~STEP_MASK) | (step_port_invert_mask & STEP_MASK));
	}
}
#endif
#ifdef WIN32
...
#ifdef STM32F103C8
  HAL_GPIO_Write(STEP_PORT, (HAL_GPIO_Read(STEP_PORT) & ~STEP_MASK) | (step_port_invert_mask & STEP_MASK));
  HAL_GPIO_Write(DIRECTION_PORT, (HAL_GPIO_Read(DIRECTION_PORT) & ~DIRECTION_MASK) | (dir_port_invert_mask & DIRECTION_MASK));
  #ifdef ENABLE_DUAL_AXIS
    st.dir_outbits_dual = dir_port_invert_mask_dual;
    HAL_GPIO_Write(STEP_PORT_DUAL, (HAL_GPIO_Read(STEP_PORT_DUAL) & ~STEP_MASK_DUAL) | (step_port_invert_mask_dual & STEP_MASK_DUAL));
    HAL_GPIO_Write(DIRECTION_PORT_DUAL, (HAL_GPIO_Read(DIRECTION_PORT_DUAL) & ~DIRECTION_MASK_DUAL) | (dir_port_invert_mask_dual & DIRECTION_MASK_DUAL));
  #endif
#endif
}
...
void stepper_init()
{
  // Configure step and direction interface pins
    ...
    TIM_Configuration(&htim2, TIM2, 1, 1, 1);

    __HAL_RCC_TIM3_CLK_ENABLE();
    TIM_Configuration(&htim3, TIM3, 1, 1, 1);
...
void TIM_Configuration(TIM_HandleTypeDef* htim, TIM_TypeDef* TIMER, uint32_t Period, uint32_t Prescaler, uint32_t PP)
{
  ...
  HAL_TIM_Base_Init(htim);  // ??? Check if != HAL_OK)

  sClockSourceConfig.ClockSource = TIM_CLOCKSOURCE_INTERNAL;
  HAL_TIM_ConfigClockSource(htim, &sClockSourceConfig); // ??? Check if != HAL_OK)

  HAL_NVIC_SetPriorityGrouping(NVIC_PRIORITYGROUP_4);

  if (htim->Instance == TIM2) {
    HAL_NVIC_ClearPendingIRQ(TIM2_IRQn);
    HAL_NVIC_SetPriority(TIM2_IRQn, PP, 1);
    HAL_NVIC_EnableIRQ(TIM2_IRQn);
  } else if (htim->Instance == TIM3) {
    HAL_NVIC_ClearPendingIRQ(TIM3_IRQn);
    HAL_NVIC_SetPriority(TIM3_IRQn, PP, 1);
    HAL_NVIC_EnableIRQ(TIM3_IRQn);
  } else if (htim->Instance == TIM4) {
    HAL_NVIC_ClearPendingIRQ(TIM4_IRQn);
    HAL_NVIC_SetPriority(TIM4_IRQn, PP, 1);
